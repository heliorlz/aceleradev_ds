...
def main():
    ...
    if file is not None:
        ...
        exploracao = pd.DataFrame({'nomes': df.columns, 'tipos': df.dtypes, 'NA #': df.isna(
        ).sum(), 'NA %': (df.isna().sum() / df.shape[0]) * 100})
        ...
        st.markdown(
            list(exploracao[exploracao['tipos'] == 'float64']['nomes']))
        ...
        percentual = st.slider(
            'Escolha o limite de percentual faltante limite para as colunas vocë deseja inputar os dados', min_value=0, max_value=100)
        lista_colunas = list(
            exploracao[exploracao['NA %'] < percentual]['nomes'])
        select_method = st.radio(
            'Escolha um metodo abaixo :', ('Média', 'Mediana'))
        ...
        if select_method == 'Média':
            ...
            st.markdown(get_table_download_link(
                df_inputado), unsafe_allow_html=True)
